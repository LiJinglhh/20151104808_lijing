...
public class TeacherController {
    ...
    public String productsAdd(String endTimeString, VoteBatch voteBatch, VoteQuestions voteQuestions, Model model, HttpServletRequest request) {
        ...
        if (cEmptydNotEmpty(voteQuestions.getQuestionCs(), voteQuestions.getQuestionDs())) {
            errQuestionNameMsg = "请先输入选项C才可以输入选项D";
            flag = false;
        }
        if (flag == false) {
            int questionNum = 1;
    ...
    public boolean cEmptydNotEmpty(String[] charsC, String[] charsD) {
        for (int i = 0; i < charsC.length; i++) {
            if ("".equals(charsC[i]) && !"".equals(charsD[i])) {
                return true;
            }
        }
        return false;
